...
def _init_(args):
    ...
    os.system(f'cp data/market77_h5.py {backup_dir}/market77_h5.py.backup')
...
def test(args, io):
    ...
    device = torch.device('cuda' if args.cuda else 'cpu')
    model = DGCNN_semseg_s3dis(args).to(device)
    model.conv9 = nn.Conv1d(256, args.classes, 1, bias=False).to(device)
    model = nn.DataParallel(model)
    # Load best checkpoint
    ckpt_path = os.path.join('outputs', args.exp_name, 'models', 'best_model.pth')
    ...
    ious = calculate_iou(all_pred, all_true, args.classes)
    miou = np.nanmean(ious)
    io.cprint(f"Test Acc {acc:.4f}, IoU_Class0 {ious[0]:.4f}, "
              f"IoU_Class1 {ious[1]:.4f}, mIoU {miou:.4f}")