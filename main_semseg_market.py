...
from util import cal_loss, dice_loss, IOStream
...
def _init_(args):
    ...
    backup_dir = os.path.join('outputs', args.exp_name, 'backup')
    os.makedirs(backup_dir, exist_ok=True)
    os.system(f'cp main_semseg_market.py {backup_dir}/main_semseg_market.py.backup')
    os.system(f'cp model.py {backup_dir}/model.py.backup')
    os.system(f'cp util.py {backup_dir}/util.py.backup')
    # Backup dataset definition file if it exists
    dataset_file = os.path.join('data', 'market77_h5.py')
    if os.path.exists(dataset_file):
        os.system(f'cp {dataset_file} {backup_dir}/market77_h5.py.backup')
...
def train(args, io):
    ...
    # Use cal_loss with class weights, same as S3DIS
    weights = torch.tensor([0.5555556, 5.0], dtype=torch.float).to(device)
    io.cprint(f"Using fixed class weights: {weights.cpu().numpy()}")
    io.cprint("Using combined CrossEntropy + Dice loss for single-object segmentation")
    criterion = lambda pred, gold: cal_loss(pred, gold, weight=weights) + dice_loss(pred, gold)

    # Resume from checkpoint if provided
    start_epoch = 0
    best_epoch = 0
    if args.resume:
        # Load full checkpoint (model + optimizer + scheduler)
        ...
        io.cprint(f"Resuming from epoch {start_epoch}, best mIoU={best_iou:.4f}")
    else:
        best_iou = 0.0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Show a single, in-place progress bar on stderr
        ...
        for batch_idx, (data, seg) in enumerate(tqdm(train_loader, **bar_kwargs), start=1):
    ...
    io.cprint(f"Training complete. Best val mIoU {best_iou:.4f} at epoch {best_epoch}")

    # Plot Loss & Accuracy (use actual number of epochs run)
